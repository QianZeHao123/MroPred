...
model_save_path = os.path.join(pt_model_dir, f"best_model_{timestamp}_test.pth")
pt_model_save_path = os.path.join(pt_model_dir, f"best_model_{timestamp}_test.pt")
...
#         weighted = torch.sum(encoder_outputs * weights, dim=1)
#         return weighted
...
class RnnModel(nn.Module):
    ...
    def __init__(
        ...
        # self.attention = SelfAttention(rnn_output_size * num_directions)
        self.multihead_attn = nn.MultiheadAttention(
            embed_dim=rnn_output_size * num_directions,
            num_heads=num_heads,
            batch_first=True
        )
        self.fc = nn.Sequential(
            nn.Linear(rnn_output_size * num_directions, 64),
            nn.ReLU(),
            nn.Dropout(0.5),
